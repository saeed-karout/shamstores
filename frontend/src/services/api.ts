...
class ApiService {
  ...
  private silentErrorPaths = [
    ...
    '/analytics/visits',
    // الكاشير يعرض أخطاء البيع بنفسه — وطابور «دون اتصال» يعيد المحاولة
    // في الخلفية، ففقاعة «خطأ في الاتصال» مع كل محاولة ضجيجٌ لا معلومة
    // (يطابق `/pos/sales/…` أيضاً، وشاشة المرتجع تعرض خطأها بنفسها)
    '/pos/sale',
    // دون اتصال تُعرض أصناف الجهاز المحفوظة — فقاعة خطأٍ مع كل حرفٍ بحث ضجيج
    '/pos/products',
    // المساعد الذكي يعرض الحصّة والترقية بنفسه — 403 للخطة المجانية حالةٌ
    // متوقّعة لا خطأ، ولا يجوز أن تُعيد التاجر إلى الرئيسية وسط المراجعة
    '/ai/',
    // تسوية التحصيل مقيّدة بالخطة: بطاقتها في تفاصيل الطلب تختفي بصمت عند
    // 403، وشاشتها تعرض الترقية والأخطاء بنفسها
    '/cod/',
  ];
...
getApiBaseUrl(): string {
 ...
 async getPublicBusiness(identifier?: string): Promise<any> {
  ...
  // التحقق: إذا لم يتم العثور على معرف، لا ترسل الطلب وأعد خطأ مفهوم
  if (!id) {
    console.warn('⚠️ Cannot fetch public business: No identifier or subdomain found.');
    return Promise.reject(new Error('No business identifier found.'));
  }
  
  console.log('🌐 Fetching public business for identifier:', id);
  // أرسل الطلب إلى المسار الصحيح فقط
  const response = await this.get(`/public/${id}`);
  return response;
}
  ...
  async getPublicCategories(): Promise<any[]> {
    const business = await this.getPublicBusiness();
