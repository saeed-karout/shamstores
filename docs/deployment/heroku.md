...
### يعمل تلقائياً في مرحلة release
...
**أمران بصلاحيتين مختلفتين عمداً:**

| | عند الفشل |
|---|---|
| `prisma:db-push` | **يُلغى النشر** — مخطّطٌ متأخّر عن الشيفرة يعني موقعاً ساقطاً |
| `seed:features` | يُسجَّل تحذير و**يستمرّ النشر** — كتالوج ناقص يعني ميزةً لا تُطلَب، لا انقطاعاً |

و`seed:features` **يضيف الناقص ولا يعدّل الموجود**، فلا يكتب فوق سعرٍ
عدّلته من اللوحة. لتحديثٍ مقصود للأوصاف: `-- --force` يدوياً.

**كان هنا نصٌّ يقول عكس ذلك** («لا تُشغّل هذا تلقائياً — قد يحذف أعمدة»).
عُكس القرار بعد أن سقط الإنتاج **أربع مرّات في يوم واحد** بنفس السبب:
عمودٌ في الشيفرة المنشورة لا وجود له في القاعدة (`P2022`)، فيسقط كل مسارٍ
يقرأ ذلك الجدول.

والسبب ليس نسياناً يُعالَج بالتذكير: `git push` إلى Heroku **ينشر فوراً**،
فأي تأخّرٍ ثانيةً واحدة بين النشر ودفع المخطّط هو نافذة انقطاع. الترتيب
الصحيح مكتوباً لا يمنعها؛ البناء يمنعها.

ومرحلة الإصدار تحلّها بنيوياً: Heroku ينفّذها **قبل** تبديل الدايْنو، و**إن
فشلت أُلغي النشر** ولم تُستبدل النسخة العاملة. فالحالة الأسوأ صارت «نشرٌ
لم يحدث» بدل «موقعٌ ساقط».

### 🛑 لا تُضِف `--accept-data-loss` إلى Procfile

هذا هو الشرط الذي يجعل ما سبق آمناً، والخوف القديم كان في محلّه بدونه.

`db push` بلا هذا العلم **يتوقّف** عند أي تغيير مدمّر بدل تنفيذه — رأيناه
يرفض إضافة قيد تفرّد لمجرّد **احتمال** التكرار. فمرحلة الإصدار تفشل، ويُلغى
النشر، وتُنبَّه. أمّا مع العلم فتصير المرحلة تحذف أعمدةً بصمت في كل نشر.

حين يتطلّب تغييرٌ حذفاً مقصوداً، عاينه ثم طبّقه يدوياً مرّةً واحدة:

```bash
heroku run --app shamstores --no-tty "npm --prefix backend run prisma:db-push -- --accept-data-loss"
```

ثمّ ادفع الشيفرة — فتمرّ مرحلة الإصدار بلا شيء تفعله.

### تشغيله يدوياً

يبقى متاحاً لنشرةٍ سابقة أو لقاعدةٍ خارج Heroku:
...
ثلاث نقاط بثلاثة جماهير — ولكل واحدة سبب وجود مختلف:

| النقطة | من يستخدمها | ماذا تفحص |
|---|---|---|
| `/health` | مسبار إقلاع Heroku | أن العملية حيّة — **سطحية عمداً** |
| `/health/ready` | المراقب الخارجي | قاعدة البيانات فعلاً · 200 أو 503 |
| `/health/detail` | السوبر أدمن | الصورة كاملة (يتطلب تسجيل دخول) |

> ⚠️ **لا تراقب `/health`.** يُرجع `ok` حتى وقاعدة البيانات ساقطة — يقيس أن
> العملية حيّة لا أن التطبيق يعمل. مراقب يفحصه يرى أخضر بينما كل تاجر عاجز
> عن استقبال طلب، وهذا أسوأ من غياب المراقبة لأنه يمنحك ثقة كاذبة.

`/health/ready` **لا تكشف تفاصيل**: نقطة مفتوحة للعالم لا تُظهر بنيتك ولا
رسائل أخطاء قاعدة بياناتك. التفاصيل في `/health/detail` خلف المصادقة.

#### ربط منبّه خارجي

أي خدمة مراقبة مجانية تكفي (UptimeRobot أو Better Stack أو ما يشبههما):

```
الرابط:    https://shamstores.com/health/ready
الفاصل:    كل ٥ دقائق
النجاح:    رمز 200
التنبيه:   بريد + واتساب إن توفّر
```

النقاط الثلاث مستثناة من وضع الصيانة — المراقب يجب أن يفحص حتى والموقع في
صيانة، وإلا ظننته ساقطاً.

#### ما لا يراه المراقب الخارجي

المراقب يرى الموقع ساقطاً، لا يرى **الأعطال الصامتة**: نسخة احتياطية فشلت
أو بريد توقّف. الموقع أخضر وتمرّ الأيام.

لذلك يُرسل النظام تنبيهاً بالبريد عند فشل النسخة اليومية. اضبط وجهته:

```bash
heroku config:set SMTP_ALERT_EMAIL=you@example.com --app shamstores
```

وافحص الصورة الكاملة يدوياً بين حين وآخر من `/health/detail` — تُظهر حالة
قاعدة البيانات والبريد والوسائط وعمر آخر نسخة وسعر الصرف في نداء واحد.

---

### النسخ الاحتياطي

> ⚠️ خطة JawsDB المجانية **بلا نسخ احتياطية**. عطل واحد بلا نسخة ينهي
> المشروع — لا يعطّله.

نسخة يومية عند 03:00 إلى Cloudflare R2، مضغوطة، مع احتفاظ ١٤ يوماً.

#### ⚠️ حاوية منفصلة، لا حاوية الوسائط

حاوية `shamstores-images` **عامة** — تخدم `cdn.shamstores.com`. والنسخة
تحوي بريد كل مستخدم وهاتفه وبصمات كلمات المرور وكل الطلبات. وضعها هناك
يسلّمها لأي زائر يخمّن اسم الملف.

لذلك **يرفض النظام العمل** ما لم تُضبط حاوية منفصلة، ويرفضه صراحةً لو
ساوت حاوية الوسائط.

**الإعداد لمرة واحدة:**

1. Cloudflare → R2 → إنشاء حاوية باسم `shamstores-backups`.
2. **لا تُفعّل الوصول العام لها ولا تربط بها نطاقاً.**
3. اضبط المتغيّر:

```bash
heroku config:set R2_BACKUP_BUCKET=shamstores-backups --app shamstores
```

يتحقق من التفعيل بسطر في سجل الإقلاع: `💾 النسخ الاحتياطي اليومي مفعّل`،
أو `⚠️ النسخ الاحتياطي معطّل` مع السبب. النسخ المعطّل بصمت أخطر من غيابه
المعلَن.

#### الأوامر

```bash
heroku run --app shamstores "node backend/scripts/admin.js backup-now"
```

```bash
heroku run --app shamstores "node backend/scripts/admin.js backup-list"
```

`backup-list` ينبّه إن تجاوز عمر أحدث نسخة ٣٠ ساعة — أي أن الجدولة متوقفة.
شغّله دورياً: نسخة لا تعرف أنها موجودة ليست نسخة.

#### الاستعادة

النسخة ملف JSON مضغوط، فيه لكل نموذج مصفوفة صفوفه. الاستعادة عملية واعية
لا أمر واحد: نزّل الملف، فُكّ الضغط، وأعد الإدراج بترتيب يحترم المفاتيح
الأجنبية (المستخدمون قبل المطاعم قبل الطلبات).

> **جرّب الاستعادة مرة على قاعدة تطوير قبل أن تحتاجها.** نسخة لم تُختبر
> استعادتها ليست نسخة بل افتراض.
...
| `P2022 column does not exist` | نُشرت شيفرة تسبق المخطّط | يجب ألّا يحدث بعد الآن — مرحلة الإصدار تدفع المخطّط قبل التبديل. إن حدث فراجع سجلّ المرحلة: هل فشلت وأُلغي النشر؟ |
| النطاق المخصص لا يفتح المتجر | لم يُضَف إلى Heroku، أو التحقق لم يكتمل | راجع القسم 8 |
| `535 BadCredentials` من Gmail | كلمة مرور التطبيق مُلصقة بمسافاتها | أرسلها 16 حرفاً متصلة — Google يعرضها مقسّمة للقراءة فقط |
| لا يصل بريد التحقق ولا خطأ في الواجهة | فشل الإرسال كان مُبتلَعاً | حُلّ: سطر خطأ في السجل وحقل `emailDelivered` في الاستجابة |
...
- [ ] `Procfile` فيه سطر `release` (يدفع المخطّط قبل تبديل الدايْنو)
