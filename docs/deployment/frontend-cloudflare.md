...
| 1 | `/api/*` أو `/socket.io/*` أو `/health*` | **Heroku** |
...
### ⚠️ `/health` يجب أن يُمرَّر أيضاً

وقع هذا فعلاً: نقاط الصحة بُنيت على `/health/*` والـ Worker يمرّر `/api/*`
وحده، فذهبت إلى الواجهة الثابتة التي تُرجع `index.html` لأي مسار — **بحالة
200**.

النتيجة أخطر من غياب المراقبة: المنبّه يرى أخضر دائماً، حتى لو سقط الخادم
بالكامل. مراقبة تكذب أسوأ من غيابها.

للتحقق بعد أي نشر للـ Worker:

```bash
curl -s https://shamstores.com/health/ready
```

يجب أن يعود `{"status":"ready"}` لا HTML.
