...
// ==================== أدوار محددة (للتوافق مع الكود القديم) ====================
...
export const verifyStoreAccess = async (
  ...
  try {
    ...
    // الموظف لا يمكنه الوصول إلى المتاجر (يمكن تعديل هذا حسب الحاجة)
    if (userRole === 'staff') {
      res.status(403).json({ 
        success: false, 
        error: 'الموظفون لا يمكنهم الوصول إلى المتاجر' 
      });
      return;
    }
...
/**
 * التحقق من صلاحية الوصول إلى طلب معين (سواء كان لمطعم أو متجر)
 */
export const verifyOrderAccess = async (
  ...
  try {
    ...
    // استيراد نموذج Order ديناميكياً
    ...
    if (userRole === 'staff') {
      // الموظف يمكنه الوصول فقط لطلبات مطعمه
      if (order.restaurantId && userRestaurantId === order.restaurantId) {
        next();
        return;
      }
