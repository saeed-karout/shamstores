// backend/scripts/migrate-features.js
...
async function runMigration() {
  ...
  try {
    ...
    const migrationSQL = `
      -- =====================================================
      -- نظام الميزات المتقدم (Features System)
      -- =====================================================
      
      -- 1. حذف الجداول القديمة إذا وجدت (بالترتيب الصحيح)
      DROP TABLE IF EXISTS extended_platform_settings;
      DROP TABLE IF EXISTS business_features;
      DROP TABLE IF EXISTS features;
      
      -- 2. إنشاء جدول الميزات
      CREATE TABLE features (
        ...
        feature_group ENUM('basic', 'marketing', 'advanced', 'payment', 'delivery', 'analytics', 'integration') DEFAULT 'basic',
      ...
      -- 3. إنشاء جدول ربط الميزات مع العملاء (بدون FOREIGN KEYS)
      ...
      -- 4. إنشاء جدول إعدادات المنصة الموسع
      CREATE TABLE extended_platform_settings (
        id CHAR(36) PRIMARY KEY DEFAULT (UUID()),
        key_name VARCHAR(100) NOT NULL UNIQUE,
        value TEXT NOT NULL,
        type ENUM('string', 'number', 'boolean', 'json', 'array') DEFAULT 'string',
        setting_group ENUM('general', 'auth', 'business', 'subscription', 'payment', 'domain', 'storage', 'delivery', 'notification', 'security', 'analytics') DEFAULT 'general',
        description TEXT NULL,
      ...
      -- 5. إدخال الميزات الأساسية
      INSERT INTO features (code, name, name_en, category, feature_group, is_core, is_active, price, is_one_time) VALUES
      ...
      -- ميزات المطاعم المتقدمة (مطاعم فقط)
      ...
      -- ميزات المتاجر المتقدمة (متاجر فقط)
      ...
      -- ميزات مشتركة مدفوعة (كلا النوعين)
      ...
      ('disable_public_ads', 'تعطيل الإعلانات في الصفحة العامة', 'Disable Public Ads', 'both', 'marketing', FALSE, TRUE, 0, FALSE),
      ...
      ('priority_support', 'دعم فني أولوية', 'Priority Support', 'both', 'basic', FALSE, TRUE, 49.99, FALSE),
      
      -- ميزات المتاجر الإضافية (متاجر فقط)
      ...
      -- 6. إدخال إعدادات المنصة الافتراضية
      INSERT INTO extended_platform_settings (key_name, value, type, setting_group, description, is_public) VALUES
      -- General Settings
    ...
    const [featuresCount] = await connection.query('SELECT COUNT(*) as count FROM features');
    const [settingsCount] = await connection.query('SELECT COUNT(*) as count FROM extended_platform_settings');
    
    console.log(`\n📊 Summary:`);
    console.log(`   - Features created: ${featuresCount[0].count}`);
    console.log(`   - Platform settings created: ${settingsCount[0].count}`);
